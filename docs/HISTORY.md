...
1. Created `debugLog.ts` module: circular buffer (max 200 entries), typed sources/severities (`DebugSource`, `DebugSeverity` exported), pub/sub with immediate delivery for late subscribers, console.error/warn interception with HMR guard (true originals stored on window) and re-entrancy guard (prevents infinite recursion), global error/unhandledrejection capture, shared `formatDebugTimestamp()` helper, report generation with URL redaction. All browser side-effects guarded with `typeof window !== 'undefined'` for SSR safety. After bridge, inline listeners are removed and buffer cleared to prevent double-capture.
...
3. Created `DebugPill.svelte`: floating debug pill with inline styles (not Tailwind — survives stylesheet failures), 3 tabs (Log with color-coded entries + structured details display, Environment with runtime info, PWA Diagnostics with active health probes and stale-run cancellation), mounted in separate `#debug-root` outside SvelteKit tree to survive app crashes. Uses Svelte `tick()` for DOM-safe auto-scroll. Log container fills available panel space (no fixed max-height). Visible textarea fallback when all clipboard methods fail (per pattern). Initialisation relies on subscriber immediate delivery. Logs boot confirmation on mount.
4. Updated `app.html`: added `#debug-root` div, pre-framework inline `<script>` with `window.__debugPushError()` bridge, 20-second loading timeout, and named listener references (`__debugInlineErrorHandler`, `__debugInlineRejectionHandler`) so the module can remove them after takeover.
5. Updated `+layout.svelte`: dynamic import of DebugPill (prevents `debugLog.ts` SSR execution), stores component reference, returns cleanup function for proper `$destroy()` on unmount. `destroyed` flag prevents creating pill after cleanup (race condition guard).
