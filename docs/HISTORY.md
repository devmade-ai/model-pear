...
1. Created `debugLog.ts` module: circular buffer (max 200 entries), typed sources/severities (`DebugSource`, `DebugSeverity` exported), pub/sub with immediate delivery for late subscribers, console.error/warn interception, global error/unhandledrejection capture, shared `formatDebugTimestamp()` helper, report generation with URL redaction. All browser side-effects guarded with `typeof window !== 'undefined'` for SSR safety.
...
5. Updated `+layout.svelte`: dynamic import of DebugPill (prevents `debugLog.ts` SSR execution), stores component reference, returns cleanup function for proper `$destroy()` on unmount.
